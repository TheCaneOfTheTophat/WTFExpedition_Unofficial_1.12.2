	...
	public ChunkScan getChunkScan(World world, ChunkCoords coords, GeneratorMethods gen) {
		...
		for (int xloop = 0; xloop < 16; xloop++) {
			...
			for (int zloop1 = 0; zloop1 < 16; zloop1++) {
				...
				//removed y>1- because if it's getting through in vanilla, there's a bug
				while (isAirAndCheck(chunk, gen, column.get(y), worldx, y, worldz))
					y--;
				...
				for (y = y - 1; y > -1; y--) {
					...
					//This is on an else if because if the current value is the first one, then it's just checking it against itself
						...
						} else if (isWall(chunk, currentPos) && airBlocks.contains(currentPos.west()))
